%% Convert
function [error_code] = convert_h5_to_mat(input_path,output_path)

try
    % identify files with ending '.h5'
    listfiles = dir([input_path filesep '*.h5']);
    ...
    for k = 1:size(listfiles,1)
        ...
        info = h5info([input_path filesep namefile]);    
        kinedata = [];
        for m = 1:3;% number of monitors
            % assign location of monitor to the number of monitor
            monitor(:,1) = h5readatt([input_path filesep namefile],'/','MonitorLabelList');
            monitor(:,2) = h5readatt([input_path filesep namefile],'/','CaseIdList');
            for e = 1:3
